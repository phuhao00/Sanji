...
impl<'a> TransformQuery<'a> {
    ...
    pub fn execute(&self) -> Vec<(Entity, &Transform)> {
        ...
        (&entities, &transforms).join().collect()
    }

    /// 按条件过滤
    pub fn filter<F>(self, predicate: F) -> Vec<(Entity, &'a Transform)>
    where
        F: Fn(&Transform) -> bool,
    {
        self.execute()
...
impl<'a> RenderableQuery<'a> {
    ...
    pub fn execute(&self) -> Vec<(Entity, &Transform, &MeshRenderer)> {
        ...
        (&entities, &transforms, &renderers).join().collect()
    }
...
impl<'a> CameraQuery<'a> {
    ...
    pub fn execute(&self) -> Vec<(Entity, &Transform, &Camera)> {
        ...
        (&entities, &transforms, &cameras).join().collect()
    }
...
impl<'a> LightQuery<'a> {
    ...
    pub fn execute(&self) -> Vec<(Entity, &Transform, &Light)> {
        ...
        (&entities, &transforms, &lights).join().collect()
    }
